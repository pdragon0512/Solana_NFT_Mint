...
export const ERButton = styled(Button)`
  position: absolute;
  width: 100%;
  height: 60px;
  margin-top: 10px;
...
const Home = (props: HomeProps) => {
  ...
  const [NftTotal, setNftTotal] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const nfts =   await Metadata.findDataByOwner(props.connection, anchorWallet.publicKey);
    console.log(anchorWallet.publicKey.toBase58())
    let treeCount : number = 0;
    nfts.forEach((nft) => {
          if(process.env.REACT_APP_CANDY_MACHINE_ID === nft.updateAuthority)
                treeCount ++;
    })
    setNftTotal(treeCount);
      // try {
                    ...
                    <MintContainer>
                      ...
                      { NftTotal !== 0 ?(
                        <>
                          ...
                          onMint={onMint}
                          />
                        <Link to="/main"><ERButton>Enter Room</ERButton></Link>
                        </>
                      ):(
                        <MintButton
                        candyMachine={candyMachine}
